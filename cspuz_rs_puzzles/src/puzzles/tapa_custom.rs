...
pub fn solve_tapa_custom(clues: &[Vec<Option<[i32; 4]>>]) -> Option<Vec<Vec<Option<bool>>>> {
    ...
    // CUSTOM: UNSHADED connected (not shaded — opposite of standard tapa)
...
pub fn serialize_problem(problem: &Problem) -> Option<String> {
    problem_to_url(combinator(), "tapa_custom", problem.clone())
}

pub fn deserialize_problem(url: &str) -> Option<Problem> {
    url_to_problem(combinator(), &["tapa", "tapa_custom"], url)
}
