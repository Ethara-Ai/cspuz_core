use cspuz_rs::graph;
use cspuz_rs::serializer::{
    problem_to_url_with_context, url_to_problem, Choice, Combinator, Context, Dict, HexInt,
    Optionalize, RoomsWithValues, Size, Spaces,
};
use cspuz_rs::solver::{count_true, BoolVarArray2D, Solver};

use super::heyawake;

pub fn solve_heyawake2(
    borders: &graph::InnerGridEdges<Vec<Vec<bool>>>,
    clues: &[Option<i32>],
) -> Option<Vec<Vec<Option<bool>>>> {
    let (h, w) = borders.base_shape();

    let mut solver = Solver::new();
    let is_black = &solver.bool_var_2d((h, w));
    solver.add_answer_key_bool(is_black);

    add_constraints(&mut solver, is_black, borders, clues, h, w);

    solver.irrefutable_facts().map(|f| f.get(is_black))
}

fn add_constraints(
    solver: &mut Solver,
    is_black: &BoolVarArray2D,
    borders: &graph::InnerGridEdges<Vec<Vec<bool>>>,
    clues: &[Option<i32>],
    h: usize,
    w: usize,
) {
    // Reuse standard heyawake constraints (connectivity, room counts, line-of-sight)
    // but heyawake base has relaxed adjacency — we override with strict
    graph::active_vertices_connected_2d(solver, !is_black);

    // STRICT adjacency: zero adjacent pairs allowed (override heyawake's relaxed version)
    solver.add_expr(!is_black.conv2d_and((1, 2)));
    solver.add_expr(!is_black.conv2d_and((2, 1)));

    // Line-of-sight constraint (copied from heyawake::add_constraints)
    for y in 0..h {
        for x in 0..w {
            if y + 2 < h && borders.horizontal[y][x] {
                let mut y2 = y + 2;
                while y2 < h && !borders.horizontal[y2 - 1][x] {
                    y2 += 1;
                }
                if y2 < h {
                    solver.add_expr(is_black.slice_fixed_x((y..=y2, x)).any());
                }
            }
            if x + 2 < w && borders.vertical[y][x] {
                let mut x2 = x + 2;
                while x2 < w && !borders.vertical[y][x2 - 1] {
                    x2 += 1;
                }
                if x2 < w {
                    solver.add_expr(is_black.slice_fixed_y((y, x..=x2)).any());
                }
            }
        }
    }

    // Room shade counting
    let rooms = graph::borders_to_rooms(borders);
    assert_eq!(rooms.len(), clues.len());
    for i in 0..rooms.len() {
        if let Some(n) = clues[i] {
            let mut cells = vec![];
            for &pt in &rooms[i] {
                cells.push(is_black.at(pt));
            }
            if n >= 0 {
                solver.add_expr(count_true(cells).eq(n));
            }
        }
    }

    // CUSTOM: Column shade balance — at most ceil(rows/2) shaded per column
    let col_cap = ((h + 1) / 2) as i32;
    for x in 0..w {
        solver.add_expr(is_black.slice_fixed_x((.., x)).count_true().le(col_cap));
    }

    // CUSTOM: Shading density 10-50%
    let total = (h * w) as i32;
    let min_shaded = (total + 9) / 10; // ceil(total * 0.1)
    let max_shaded = total / 2; // floor(total * 0.5)
    solver.add_expr(is_black.count_true().ge(min_shaded));
    solver.add_expr(is_black.count_true().le(max_shaded));
}

pub(super) type Problem = (graph::InnerGridEdges<Vec<Vec<bool>>>, Vec<Option<i32>>);

fn combinator() -> impl Combinator<Problem> {
    Size::new(RoomsWithValues::new(Choice::new(vec![
        Box::new(Optionalize::new(HexInt)),
        Box::new(Spaces::new(None, 'g')),
        Box::new(Dict::new(Some(-1), ".")),
    ])))
}

pub fn serialize_problem(problem: &Problem) -> Option<String> {
    let height = problem.0.vertical.len();
    let width = problem.0.vertical[0].len() + 1;
    problem_to_url_with_context(
        combinator(),
        "heyawake2",
        problem.clone(),
        &Context::sized(height, width),
    )
}

pub fn deserialize_problem(url: &str) -> Option<Problem> {
    url_to_problem(combinator(), &["heyawake", "heyawake2"], url)
}
