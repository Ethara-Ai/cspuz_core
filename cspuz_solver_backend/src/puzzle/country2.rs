...
use cspuz_rs::graph;
use cspuz_rs_puzzles::puzzles::country2;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    let (empty, (borders, clues)) = country2::deserialize_problem(url).ok_or("invalid url")?;
    let is_line = country2::solve_country2(empty, &borders, &clues);
