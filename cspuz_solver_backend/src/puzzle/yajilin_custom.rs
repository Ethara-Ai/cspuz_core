...
use cspuz_rs_puzzles::puzzles::yajilin_custom;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    use cspuz_rs::items::Arrow;

    let (_outside, problem) = yajilin_custom::deserialize_problem(url).ok_or("invalid url")?;
    let ans = yajilin_custom::solve_yajilin_custom(&problem);
