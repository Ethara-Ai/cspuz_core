...
use cspuz_rs::graph;
use cspuz_rs_puzzles::puzzles::heyawake2;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    let (borders, clues) = heyawake2::deserialize_problem(url).ok_or("invalid url")?;
    let is_black = heyawake2::solve_heyawake2(&borders, &clues);
