...
use cspuz_rs_puzzles::puzzles::nurikabe_custom;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    let problem = nurikabe_custom::deserialize_problem(url).ok_or("invalid url")?;
    let ans = nurikabe_custom::solve_nurikabe_custom(&problem);
